...
import { getQuestionnairePrompts } from "@/lib/questionnaire";
...
const welcomeCopy: Record<UiLanguage, WelcomeCopy> = {
  english: {
    ...
    continueHint: "Next: You'll start with Communication.",
  ...
  spanish: {
    ...
    continueHint: "Siguiente: comenzará con la sección Comunicación.",
...
const reflectionCopy: Record<UiLanguage, ReflectionCopy> = {
  english: {
    ...
    subtitle: "Answer what you can. Examples are helpful.",
    completionMessage: "You created a caregiver guide for your loved one.",
    caringFor: (name) => `Caring for ${name}`,
    enterAtLeastOneResponse: "Add at least one response before continuing.",
    backButton: "Back",
    ...
    audioLimitNotice: "Keep each recording to 1 minute or less.",
    ...
    recordingTooLarge:
      "That recording is too long. Keep recordings to 1 minute or less, or type the rest.",
    ...
    audioLimitReached: (languageLabel, isEnglish) =>
      isEnglish
        ? "1-minute limit reached. Transcribing now and retrying automatically if the transcription service is busy..."
        : `1-minute limit reached. Translating ${languageLabel} speech into English now and retrying automatically if the transcription service is busy...`,
    recordingStatus: (current, max) => `Recording ${current} of ${max}.`
  },
  spanish: {
    title: "Preguntas para personas cuidadoras",
    subtitle: "Responda lo que pueda. Los ejemplos son útiles.",
    completionMessage: "Creó una guía de cuidados para su ser querido.",
    caringFor: (name) => `Cuidando a ${name}`,
    enterAtLeastOneResponse: "Agregue al menos una respuesta antes de continuar.",
    backButton: "Atrás",
    ...
    audioLimitNotice: "Mantenga cada grabación en 1 minuto o menos.",
    ...
    recordingTooLarge:
      "La grabación es demasiado larga. Mantenga cada grabación en 1 minuto o menos, o escriba el resto.",
    ...
    audioLimitReached: (languageLabel, isEnglish) =>
      isEnglish
        ? "Se alcanzó el límite de 1 minuto. Se transcribirá ahora y se volverá a intentar automáticamente si el servicio de transcripción está ocupado..."
        : `Se alcanzó el límite de 1 minuto. La voz en ${languageLabel} se traducirá al inglés ahora y se volverá a intentar automáticamente si el servicio de transcripción está ocupado...`,
    recordingStatus: (current, max) => `Grabando ${current} de ${max}.`
  },
  mandarin: {
    title: "照护者问题",
    subtitle: "能回答多少就回答多少。示例会有帮助。",
    completionMessage: "您已为亲人创建了一份照护指南。",
    caringFor: (name) => `照护 ${name}`,
    enterAtLeastOneResponse: "请至少填写一项回答后再继续。",
    backButton: "返回",
    continueButton: "继续",
    ...
    audioLimitReached: (languageLabel, isEnglish) =>
      isEnglish
        ? "已达到 1 分钟上限。现在开始转录；如果转录服务正忙，系统会自动重试。"
        : `已达到 1 分钟上限。现在开始把${languageLabel}语音翻译成英文；如果转录服务正忙，系统会自动重试。`,
...
export function getLocalizedReflectionPrompts(language: UiLanguage): ReflectionPrompt[] {
  return getQuestionnairePrompts(language);
}
