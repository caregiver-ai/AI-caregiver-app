      ...
      <div className="space-y-3">
        ...
        {record.fields.length === 0 ? (
          <p className="rounded-2xl border border-dashed border-border bg-white px-4 py-3 text-sm text-slate-500">
            Add at least one detail or note before saving.
          </p>
        ) : null}
...
export function CareRecordsWorkspace() {
    ...
    try {
      ...
      setStatus(
        data.suggestions?.length
          ? "Review the suggested records below before saving."
  ...
  async function handleSaveSuggestion(tempId: string, record: EditableRecord) {
    ...
    if (!hasRecordContent(compacted)) {
      setError("Review the record before saving.");
      return;
    }
  ...
  return (
    <AppShell
      title="Care Records"
      subtitle="Type information or upload one image/PDF. AI suggests records, and only reviewed details are saved."
    >
      <div className="space-y-6">
        ...
        {!error && status ? <StatusBanner tone="success">{status}</StatusBanner> : null}

        <section className="space-y-4 rounded-3xl border border-border bg-canvas px-4 py-4">
          <div>
          ...
          <section className="space-y-4">
            ...
            {suggestions.map((suggestion) => (
              <RecordForm
                ...
                saveLabel={saving ? "Saving..." : "Save reviewed record"}
                onCancel={() =>
