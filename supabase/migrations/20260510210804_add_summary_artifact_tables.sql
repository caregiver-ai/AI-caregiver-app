...
create index if not exists idx_summary_facts_session_hash
  on public.summary_facts(session_id, source_turns_hash);

create index if not exists idx_summary_section_summaries_session_id
  on public.summary_section_summaries(session_id);

create index if not exists idx_summary_section_summaries_session_hash
  on public.summary_section_summaries(session_id, source_turns_hash);
