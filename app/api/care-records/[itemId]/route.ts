...
export async function PATCH(
  ...
  const normalizedItem = normalizeCareRecordItemInput(body.item);
  if (!hasCareRecordContent(normalizedItem)) {
  ...
  try {
    ...
    const item = await updateCareRecordItem(supabase, workspace.id, params.itemId, normalizedItem);
